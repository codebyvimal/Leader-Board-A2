...
import { Trophy, Map, Activity, User, Shield, LogOut, Cpu } from "lucide-react";
...
export function Sidebar() {
  ...
  // Build the mobile items array — includes admin if allowed
  const mobileItems = [
    ...navItems,
    ...(adminAllowed ? [{ name: "Control", href: "/admin", icon: Shield }] : []),
  ];
  ...
  return (
    <>
      ...
      <nav
        className="lg:hidden fixed bottom-0 left-0 right-0 border-t border-[var(--line)] bg-[var(--bg-1)]/96 backdrop-blur-2xl z-[58] flex items-stretch justify-around progress-grid shadow-[0_-10px_30px_rgba(0,0,0,0.32)]"
        style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)", minHeight: "68px" }}
      >
        {mobileItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex flex-col items-center justify-center gap-0.5 py-2 flex-1 transition-all duration-200 relative",
                isActive
                  ? "text-[var(--gold)]"
      ...
      {/* ===== DESKTOP SIDEBAR ===== */}
      <aside className="w-[10rem] h-[100dvh] fixed left-0 top-0 border-r border-[var(--line)] glass-panel progress-grid flex-col pt-6 pb-5 px-2 z-50 shadow-[4px_0_16px_rgba(0,0,0,0.5)] hidden lg:flex">
